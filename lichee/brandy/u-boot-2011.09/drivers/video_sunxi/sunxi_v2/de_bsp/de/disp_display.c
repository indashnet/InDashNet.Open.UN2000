...
//	num_scalers = bsp_disp_feat_get_num_scalers();
	...
	if(cur_line > start_delay-4) {
		/* may result in err, stat err cnt. todo? */
	}
...
s32 bsp_disp_get_screen_width_from_output_type(u32 screen_id, u32 output_type, u32 output_mode)
{
	...
	if(DISP_OUTPUT_TYPE_LCD == output_type) {
		...
		/* todo: add other output device res */
	}
...
s32 bsp_disp_lcd_set_panel_funs(char *name, disp_lcd_panel_fun * lcd_cfg)
{
	...
	if(panel_reg_count != 0) {
		gdisp.lcd_registered = 1;
		DE_INF("gdisp.lcd_registered = 1\n");
	}
...
s32 bsp_disp_get_lcd_registered(void)
{
...
s32 bsp_disp_set_hdmi_video_info(u32 screen_id, disp_video_timing *video_info)
{
	struct disp_hdmi* hdmi;
	hdmi = disp_get_hdmi(screen_id);
	if(!hdmi) {
	...
	if(hdmi->set_video_info)
	    return hdmi->set_video_info(hdmi, video_info);
