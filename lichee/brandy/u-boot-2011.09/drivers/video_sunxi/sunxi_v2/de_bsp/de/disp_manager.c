...
s32 disp_mgr_clk_init(struct disp_manager *mgr)
{
	...
	pll_freq = get_src_freq(mgrp->clk.clk_src);
	mclk_div = 1;
	while((pll_freq / mclk_div) > 300000000) {
		mclk_div ++;
	}

	de_top_clk_div(mgrp->clk.clk, mclk_div);
	return 0;
}
...
s32 disp_mgr_clk_enable(struct disp_manager *mgr)
{
	...
	de_top_clk_enable(mgrp->clk.clk, CLK_ON);
	
	mod_clk_enable(DISP_MOD_TOP, CLK_ON);
...
s32 disp_mgr_clk_disable(struct disp_manager *mgr)
{
	...
	de_top_clk_enable(mgrp->clk.clk, CLK_OFF);
	mgrp->clk.enabled = 0;
	return 0;
}
...
s32 disp_init_mgr(__disp_bsp_init_para * para)
{
	...
	for(screen_id=0; screen_id<num_screens; screen_id++) {
		struct disp_manager *mgr = &mgrs[screen_id];
		struct disp_manager_private_data *mgrp = &mgr_private[screen_id];
		...
		case 0:
			...
			mgrp->clk.clk = DISP_MOD_BE0;
			mgrp->clk.clk_src = CLK_BE_SRC;
