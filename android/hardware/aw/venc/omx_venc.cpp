...
omx_venc::omx_venc()
{
    ...
    fg2d = open(G2D_DEV, O_RDWR);
    if (fg2d < 0)
    {
        ALOGE("open %s failed, fg2d=%d", G2D_DEV, fg2d);
        fg2d = -1;
    }
	    ...
	    {   switch((VIDENC_CUSTOM_INDEX)paramIndex)
	    	{
	    		case VideoEncodeCustomParamStoreMetaDataInBuffers:
				{
					...
					else
					{
					    m_useMetaDataInBuffers = ((StoreMetaDataInBuffersParams*)paramData)->bStoreMetaData;
				    	    ALOGV(" COMPONENT_SET_PARAMETER: VideoEncodeCustomParamStoreMetaDataInBuffers %d", m_useMetaDataInBuffers);
					}
				         break;			
				}
				...
				case VideoEncodeCustomParamPrependSPSPPSToIDRFrames:
				{
					...
					else
					{
					    ...
					    ALOGV(" COMPONENT_SET_PARAMETER: VideoEncodeCustomParamPrependSPSPPSToIDRFrames %d", m_prependSPSPPSToIDRFrames);
					}
			    	        break;
				}
				...
				case VideoEncodeCustomParamEnableAndroidNativeBuffers:
				{
					...
					else
					{
					    ...
					    ALOGV(" COMPONENT_SET_PARAMETER: VideoEncodeCustomParamEnableAndroidNativeBuffers m_useAndroidBuffer %d", m_useAndroidBuffer);
					}
			    	        break;
				}
			        default:
							...
							if(pSelf->m_sInPortFormat.eColorFormat == OMX_COLOR_FormatAndroidOpaque)
							{
								...
								if(pSelf->m_input_buffer_vir  == NULL)
								{
									pSelf->m_input_buffer_vir = (OMX_U8 *)cedar_sys_phymalloc_map(pSelf->m_sInPortDef.format.video.nFrameWidth*pSelf->m_sInPortDef.format.video.nFrameHeight*4, 1024);
                                                                        ALOGD("vir %d", pSelf->m_input_buffer_vir);
									if(pSelf->m_input_buffer_vir == NULL)
									{
									...
									pSelf->m_input_buffer_phy= (OMX_U8 *)cedarv_address_vir2phy(pSelf->m_input_buffer_vir);
                                                                        ALOGD("phy %d", pSelf->m_input_buffer_phy);
								}
							}
							else
							{
								if(pSelf->m_input_buffer_vir  == NULL)
								{
									// pSelf->m_input_buffer_vir = (OMX_U8 *)cedar_sys_phymalloc_map(pSelf->m_sInPortDef.format.video.nFrameWidth*pSelf->m_sInPortDef.format.video.nFrameHeight*3/2,1024);
                                                                        // because encoder can not transform bt601 to yuv correctly, so we need a buffer to transfrom manually and specific that it is a yuv buffer for encoder.
									pSelf->m_input_buffer_vir = (OMX_U8 *)cedar_sys_phymalloc_map(pSelf->m_sInPortDef.format.video.nFrameWidth*pSelf->m_sInPortDef.format.video.nFrameHeight*4, 1024);
                                                                        ALOGD("vir %d", pSelf->m_input_buffer_vir);
									if(pSelf->m_input_buffer_vir == NULL)
									{
        	...
        	else
        	{
				...
				if(pSelf->m_input_step == STEP_QEQUEST_INPUT_BUFFER)
				{
					...
					if(pSelf->m_useMetaDataInBuffers)
					{
						...
						buffer_point = (unsigned char *)((pInBufHdr->pBuffer+pInBufHdr->nOffset) + 4);
					}
					else
					{
						buffer_point = (unsigned char *)(pInBufHdr->pBuffer+pInBufHdr->nOffset);
					}

					size_Y = pSelf->m_sInPortDef.format.video.nFrameWidth*pSelf->m_sInPortDef.format.video.nFrameHeight;
					size_C = size_Y>>1;

					pBufY = pSelf->m_input_buffer_vir;
					pBufC = pSelf->m_input_buffer_vir + size_Y;
					phy_y = pSelf->m_input_buffer_phy;
					phy_c = pSelf->m_input_buffer_phy + size_Y;

					if(pSelf->m_useAndroidBuffer)
					{
						int res;
						buffer_handle_t imgBuffer = *(buffer_handle_t*)(buffer_point);
						void *PhyAddr;
    				                int phyaddress;
                                                int ret;
						...
						unsigned char *img;
								...
								rect, (void**)&img);
						if (res != OK) {
						    ALOGE("%s: Unable to lock image buffer %p for access", __FUNCTION__, imgBuffer);
							  GraphicBufferMapper::get().unlock(imgBuffer);
						}		
                    ...
                    #if USED_ION
						{
							...
							if( fd != -1 )
							{
									if (private_handle_t::validate(imgBuffer) < 0)
									{
										ALOGE("Locking invalid buffer returning error");
									}
									else
									{
										hnd = (private_handle_t*)(imgBuffer);
										ion_import(fd,hnd->share_fd, &handle_ion);
										phy_adr= ion_getphyadr(fd,(void *)(handle_ion));
										phyaddress = phy_adr;
										//ALOGD("++++++++phyaddress: %d\n", phyaddress);
									}
									ion_close(fd);
                    ...
                    #else		
						GraphicBufferMapper::get().get_phy_addess(imgBuffer, &PhyAddr);
    				                 phyaddress = (int)PhyAddr;
                    #endif
    				
						if(phyaddress >= 0x40000000)
						    phyaddress -= 0x40000000;	
			                        else if(phyaddress < 0x40000000)
			                            phyaddress += 0x40000000;	
						...
						if(pSelf->m_sInPortFormat.eColorFormat == OMX_COLOR_FormatAndroidOpaque) 
						{
				                        ...
				                        if (pSelf->fg2d >= 0)
				                        {
				                            unsigned char* _phy_y;
				                            unsigned char* _phy_u;
				                            //unsigned char* _phy_v;
				                            long arr[2];
				                            g2d_blt blit;

				                            int _Width = pSelf->m_sInPortDef.format.video.nFrameWidth;
									        int _Height = pSelf->m_sInPortDef.format.video.nFrameHeight;
				                            
				                            if((int)phy_y < 0x40000000)
				                                _phy_y = (unsigned char*)((unsigned char*)phy_y + 0x40000000);
				                            else
				                                _phy_y = phy_y;
				                            
				                            _phy_u = _phy_y + (_Width*_Height);
				                            //_phy_v = _phy_u + (_Width*_Height)/4;
				                            
				                            blit.flag = G2D_BLT_NONE; 
				                            blit.src_image.addr[0] = (__u32)phyaddress; 
				                            blit.src_image.w = _Width;   
				                            blit.src_image.h = _Height;    
				                            blit.src_image.format = G2D_FMT_ABGR_AVUY8888;    // G2D_FMT_ARGB_AYUV8888
				                            blit.src_image.pixel_seq = G2D_SEQ_NORMAL; 
				                            blit.src_rect.w = _Width;
				                            blit.src_rect.h = _Height;
				                            blit.src_rect.x = 0;
				                            blit.src_rect.y = 0; 
				                            blit.dst_image.addr[0] = (__u32)_phy_y;
				                            blit.dst_image.addr[1] = (__u32)_phy_u;
				                            //blit.dst_image.addr[2] = (__u32)_phy_v;
				                            blit.dst_image.w = _Width;
				                            blit.dst_image.h = _Height;
				                            ...
				                            blit.color = 0xFF; 
				                            blit.alpha = 0x89; 
				                            if((ret=ioctl(pSelf->fg2d, G2D_CMD_BITBLT, (unsigned long)(&blit))) < 0) 
				                            { 
				                            	ALOGE("fg2d: G2D_CMD_BITBLT failed!"); 
				                            } 
				                        }else {
				                            ALOGE("fg2d: open fg2d failed!"); 
				                        }
                                                    #else
                                                        memcpy(pBufY, img, size_Y*4);
							cedarx_cache_op((long)pBufY, (long)(pBufY + size_Y*4), 0);
                                                    #endif
							
						}else {
							memcpy(pBufY, buffer_point,size_Y);
							memcpy(pBufC, buffer_point + size_Y,size_C);
							cedarx_cache_op((long)pBufY,(long)(pBufY + size_Y + size_C), 0);
						}
