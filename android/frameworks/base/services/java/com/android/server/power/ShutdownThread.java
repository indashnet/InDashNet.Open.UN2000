...
public final class ShutdownThread extends Thread {
    ...
    public static void rebootSafeMode(final Context context, boolean confirm) {
		...
		private void waitwifishutdown(int timeout) {
		    final long endTime = SystemClock.elapsedRealtime() + timeout;
		    boolean wifiOff = false;

		    Log.i(TAG, "Waiting for Wifi off");
		    while (SystemClock.elapsedRealtime() < endTime) {
		    if (!wifiOff) {
		        try {
		            wifiOff = (mWifiManager.getWifiState() == WifiManager.WIFI_STATE_DISABLED)?true:false;
		        } catch (Exception e) {
		            Log.e(TAG, "RemoteException during wait wifi shutdown", e);
		            wifiOff = true;
		        }
		    }
		    if ( wifiOff ) {
		        Log.i(TAG, "Wifi shutdown complete.");
		        break;
		    }
    ...
    public void run() {
		...
		waitwifishutdown(MAX_RADIO_WAIT_TIME);
    	killRemoveActivity(mContext);
    	killRemoveService(mContext);
