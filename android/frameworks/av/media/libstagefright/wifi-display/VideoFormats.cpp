...
void VideoFormats::miracastAdapter()
{
     FILE *fp1= NULL;
     FILE *fp2 = NULL; 
     char buf1[64];
     char buf2[64]; 
     int dram_clk = 0, dram_bus_width = 0; 
     mMiracastLevel = 1;  //* 0:720p p30; 1:720p p24; 2:480p p30; 3:480p p24;

     memset(buf1, 0, 64);
     memset(buf2, 0, 64);
	 
     fp1 = fopen("/sys/class/bandwidth/dram_clk", "r");
     if(fp1 == NULL)
     {
         ALOGD("fopen dram clk failed!");
     }
     fp2 = fopen("/sys/class/bandwidth/dram_bus_width", "r");
     if(fp2 == NULL)
     {
         ALOGD("fopen dram bus width failed!");
     }
     fread(buf1, 1, 64, fp1);
     fread(buf2, 1, 64, fp2);
     dram_clk = atoi(buf1);
     dram_bus_width = atoi(buf2);
     ALOGD("dram clock:%d, dram bus width:%d", dram_clk, dram_bus_width);
     if((dram_clk > 0) && (dram_bus_width > 0))
     {
	     if((dram_clk >= 32) && (dram_bus_width >= 432))
	     {
	         mMiracastLevel = 0;
	     }
	     else if((dram_clk >= 32) && (dram_bus_width >= 384))
	     {
	         mMiracastLevel = 1;
	     }
	     else
