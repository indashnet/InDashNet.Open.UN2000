...
#define SUPPORT_3D_DOUBLE_STREAM (0)
...
#define GET_CALLING_PID        (IPCThreadState::self()->getCallingPid())
void getCallingProcess(char *name)
{
       char proc_node[128];
...
bool IsCTSTest()
{
    ...
    getCallingProcess(mCallingProcess);	
...
//	mMaxOutputHeight = 576;
	...
	if(outputSetting & CEDARX_OUTPUT_SETTING_MODE_PLANNER) {
        ...
        if(IsCTSTest() == false)
        {
            mMaxOutputWidth  = 2048;
	    mMaxOutputHeight = 1024;
        }
...
//	}
    ...
    {
        if(1 == mInitRotation || 3 == mInitRotation)    //in gui_request_frame_mode, only use initRotation.
        {
            LOGD("GUI_mode_for_android42:init rotate[%d] in StagefrightVideoRenderInit()!", mInitRotation);
            nAppVideoWidth = height;
			nAppVideoHeight = width;
        }
    ...
    meta->setInt32(kKeyRotation, mInitRotation);
...
#if 1
		if((uint32_t)v3d_width * v3d_height != mVideo3dInfo.width * mVideo3dInfo.height)
		{
