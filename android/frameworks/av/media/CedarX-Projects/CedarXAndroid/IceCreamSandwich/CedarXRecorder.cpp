...
#else
status_t CedarXRecorder::setCamera(const sp<ICamera>& camera, const sp<ICameraRecordingProxy>& proxy, const String16& clientName, uid_t clientUid)
{
    LOGV("setCamera");
    ...
    if ((err = isCameraAvailable(camera, proxy, mCameraId,
...
status_t CedarXRecorder::prepare() 
{
	...
	if (mVideoSource <= VIDEO_SOURCE_CAMERA)
	{
		...
		else if (!strncmp(mCameraHalVersion, "30", 2))
		{
			...
			if(capture_fmt_str != NULL) {
				...
				LOGD("capture_fmt_str: %s", capture_fmt_str);
				capture_fmt = 1;
			}
		}
		else
	...
	vInfo.qp_min            = 10;
	vInfo.picEncmode		= 0;
	vInfo.is_mjpeg_source 	= capture_fmt;
#endif
        ...
        int32_t msgType, const sp<IMemory> &data) 
{
	...
	if(buf.format == V4L2_PIX_FMT_JPEG || buf.format == V4L2_PIX_FMT_MJPEG) {
