...
static int mkv_probe(media_probe_data_t *p) {
    unsigned long long total = 0;
    int len_mask = 0x80, size = 1, n = 1;
    unsigned char probe_data[] = { 'm', 'a', 't', 'r', 'o', 's', 'k', 'a' };
    unsigned char probe_data_webm[] = { 'w', 'e', 'b', 'm' };
    ...
    if (p->buf_size < 4 + size + (long long)total)
    	return 0;
    ...
    for (n = 4 + size; n <= 4 + size + (long long)total - sizeof(probe_data); n++)
    {
    ...
    for (n = 4 + size; n <= 4 + size + total - sizeof(probe_data_webm); n++)
    {
        if (!memcmp(&p->buf[n], probe_data_webm, sizeof(probe_data_webm)))
        {
            return AVPROBE_SCORE_MAX;
        }
    }
