...
public:
	...
	virtual Rect getCrop(){
		Parcel data, reply;
        data.writeInterfaceToken(IGraphicBufferConsumer::getInterfaceDescriptor());
		status_t result = remote()->transact(GET_CROP, data, &reply);
		Rect getRect;
		getRect.left = reply.readInt32();
		getRect.top = reply.readInt32();
		getRect.right = reply.readInt32();
		getRect.bottom = reply.readInt32();
		return getRect;
	}
	...
	virtual uint32_t getCurrentTransform(){
		Parcel data, reply;
        data.writeInterfaceToken(IGraphicBufferConsumer::getInterfaceDescriptor());
		status_t result = remote()->transact(GET_CURRENT_TRANSFORM, data, &reply);
	...
	virtual int getCurrentScalingMode(){
		Parcel data, reply;
        data.writeInterfaceToken(IGraphicBufferConsumer::getInterfaceDescriptor());
		status_t result = remote()->transact(GET_CURRENT_SCALINGMODE, data, &reply);
	...
	virtual int64_t getTimestamp(){
		Parcel data, reply;
        data.writeInterfaceToken(IGraphicBufferConsumer::getInterfaceDescriptor());
		status_t result = remote()->transact(GET_TIMESTAMP, data, &reply);
        ...
        uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
		...
		case GET_CROP:{
			CHECK_INTERFACE(IGraphicBufferConsumer, data, reply);
			Rect resRect = getCrop();
			reply->writeInt32(resRect.left);
			reply->writeInt32(resRect.top);
			reply->writeInt32(resRect.right);
			reply->writeInt32(resRect.bottom);
			return NO_ERROR;
		}break;
		case GET_CURRENT_TRANSFORM:{
			CHECK_INTERFACE(IGraphicBufferConsumer, data, reply);
			int32_t ret = getCurrentTransform();
			reply->writeInt32(ret);
			return NO_ERROR;
		}break;
		case GET_CURRENT_SCALINGMODE:{
			CHECK_INTERFACE(IGraphicBufferConsumer, data, reply);
			int32_t ret = getCurrentScalingMode();
			reply->writeInt32(ret);
			return NO_ERROR;
		}break;
		case GET_TIMESTAMP:{
