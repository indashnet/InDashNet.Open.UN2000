...
int Surface::setCrop(Rect const* rect)
{
	...
	status_t err = mGraphicBufferProducer->setCrop(*rect);
    return NO_ERROR;
}
...
int Surface::setBuffersDimensions(int w, int h)
{
	...
	status_t err = mGraphicBufferProducer->setCrop(Rect(w, h));
    return NO_ERROR;
}
...
int Surface::setScalingMode(int mode)
{
	...
	status_t err = mGraphicBufferProducer->setCurrentScalingMode(mode);
    return NO_ERROR;
}
...
int Surface::setBuffersTransform(int transform)
{
	...
	status_t err = mGraphicBufferProducer->setCurrentTransform(transform);
    return NO_ERROR;
}
...
int Surface::setBuffersTimestamp(int64_t timestamp)
{
	...
	status_t err = mGraphicBufferProducer->setTimestamp(timestamp);
    return NO_ERROR;
}
...
int Surface::dispatchSetParameter(va_list args)
{
...
int Surface::dispatchGetParameter(va_list args)
{
    int cmd = va_arg(args,int);
...
int Surface::setParameter(uint32_t cmd,uint32_t value) 
{
    ALOGV("Surface::setParameter %d,%d",cmd,value);
    
    return mGraphicBufferProducer->setParameter(cmd,value);
}

int Surface::getParameter(uint32_t cmd) 
{
    ALOGV("Surface::setParameter");
    
    return mGraphicBufferProducer->getParameter(cmd);
}
