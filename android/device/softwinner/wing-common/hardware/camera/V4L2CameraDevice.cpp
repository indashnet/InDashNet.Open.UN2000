...
bool V4L2CameraDevice::previewThread()
{
	...
	// preview
	//solution for limit of DRAM widthbench at 1080p 30fps,by henrisk,2013.9.13
#if SCALE_PREVIEW_ENABLE
	//scale the perview size for 1080p@30fps,when the framerate is 30fps and the size is 1920x1080,scale the preview sizes to 960x540
	if(mFrameWidth == 1920 && mFrameHeight == 1080/* && mFrameRate  == 30*/)
	{
		//do the thingds in Previewwindows.cpp
		mPreviewWindow->onNextFrameAvailable((void*)pbuf);
	}
	else 
		mPreviewWindow->onNextFrameAvailable((void*)pbuf);
#elif DYNAMIC_RATE_PREVIEW_ENABLE
	//dynamic framerate for 1080p@30fps,when the framerate is 30fps and the size is 1920x1080,set the framerate is 15~30;
	if(mFrameWidth == 1920 && mFrameHeight == 1080&& mFrameRate  == 30)
	{
