...
#define  GPS_CHANNEL_NAME  "ttyS5"
#define  GPS_GPIO_DEV_NAME      "/sys/class/gpio_sw/PI13/data"   
...
#if 0	//NMEA sentence for debug
	if ( !memcmp(r->in, "$GPGLL", 6) ) {
		int size = sizeof("$GPGLL,2232.51412,N,11359.37190,E,064606.00,A,A*6A");
		memcpy (r->in, "$GPGLL,2232.51412,N,11359.37190,E,064606.00,A,A*6A", size);
		r->pos = size;
	}
	if ( !memcmp(r->in, "$GPRMC", 6) ) {
		int size = sizeof("$GPRMC,064607.00,A,2232.51417,N,11359.37191,E,0.088,,310713,,,A*71");
		memcpy (r->in, "$GPRMC,064607.00,A,2232.51417,N,11359.37191,E,0.088,,310713,,,A*71", size);
		r->pos = size;
	}
	if ( !memcmp(r->in, "$GPVTG", 6) ) {
		int size = sizeof("$GPVTG,,T,,M,0.088,N,0.163,K,A*27");
		memcpy (r->in, "$GPVTG,,T,,M,0.088,N,0.163,K,A*27", size);
		r->pos = size;
	}
	if ( !memcmp(r->in, "$GPGGA", 6) ) {
		int size = sizeof("$GPGGA,064607.00,2232.51417,N,11359.37191,E,1,08,1.14,38.6,M,-2.5,M,,*71");
		memcpy (r->in, "$GPGGA,064607.00,2232.51417,N,11359.37191,E,1,08,1.14,38.6,M,-2.5,M,,*71", size);
		r->pos = size;
	}
	if ( !memcmp(r->in, "$GPGSA", 6) ) {
		int size = sizeof("$GPGSA,A,3,01,07,11,08,09,28,19,20,,,,,1.95,1.14,1.59*01");
		memcpy (r->in, "$GPGSA,A,3,01,07,11,08,09,28,19,20,,,,,1.95,1.14,1.59*01", size);
		r->pos = size;
	}
	if ( !memcmp(r->in, "$GPGSV", 6) ) {
		int size = sizeof("$GPGSV,3,1,09,01,78,085,25,07,67,224,28,08,56,302,40,09,48,299,34*70");
		memcpy (r->in, "$GPGSV,3,1,09,01,78,085,25,07,67,224,28,08,56,302,40,09,48,299,34*70", size);
		r->pos = size;
	}
#endif
                ...
                if (fd == control_fd)
                {
                    ...
                    else if (cmd == CMD_START)
					{
                        if (!started)
						{
                            ...
                            state->tmr_thread = state->callbacks.create_thread_cb("ublox_gps_tmr", gps_timer_thread, state);
                            if (!state->tmr_thread)
                            {
...
static void gps_timer_thread( void*  arg )
{
	...
	ALOGE("=======> ! gps entered timer thread");
...
//________`
	...
	//sleep_val = state->fix_freq;
        sleep_val = 1;	//________

	GPS_STATE_UNLOCK_FIX(state);
	D("============> need_sleep %d, sleep_val %d ", need_sleep, sleep_val);
	// RGE 2011-02-08
	if (need_sleep)
	{
		sleep(sleep_val);
...
gps_state_init( GpsState*  state )
{
    ...
    //if (property_get("ro.kernel.android.gps",device,"") == 0) {
    ...
    do
     {
		...
		if(state->fd < 0){
			ALOGE("open '%s' error(%d)", device, errno);
		}
		usleep (100000);
    ...
    if ( isatty( state->fd ) )
	{
     ...
     //////////////////////////////////////////////////////////////
    resetfd = open(GPS_GPIO_DEV_NAME, O_RDWR);                               // Gps_GPIO
    ALOGE("gps reset fd =  %d",resetfd);

    if (resetfd < 0) {
        ALOGE("gps_state_init : Couldn't open reset fb : %s", GPS_GPIO_DEV_NAME);
        close( resetfd ); 
        return;
    }
    ret = sprintf(buf,"%d",0);
    ret = write(resetfd, buf, ret);
    usleep(1000*1000);
    ret = sprintf(buf,"%d",1);
    ret = write(resetfd, buf, ret);
 //////////////////////////////////////////////////////////////
	...
	state->thread = state->callbacks.create_thread_cb("ublox_gps", gps_state_thread, state);
        if (!state->thread)
	{
...
static const void* ublox_gps_get_extension(const char* name)
{
    return NULL;
}
...
static const GpsInterface  ubloxGpsInterface = {
    ...
    .inject_time = ublox_gps_inject_time,
...
static void gps_dev_set_message_rate(int fd, int rate)
{
  ...
  DFR("gps set to nmea msg rate %d ", rate);
