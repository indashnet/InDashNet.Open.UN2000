...
/*
 * This will write boot0 and u-boot
 */
    ...
    void *cookie)
{
    ...
    int isBoot = NONE;
    /*
     * --we will write boot0 and u-boot--
     * --Extend array size,just want read and write boot0 or u-boot as once--
     */
    if (strstr(pEntry->fileName,"boot0_nand")) {
        isBoot = BOOT0;
        arrayLen = 640 * 1024;
    } else if (strstr(pEntry->fileName,"u-boot")) {
        isBoot = UBOOT;
        arrayLen = 1024 * 1024;
    }
